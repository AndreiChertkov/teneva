...
class ANOVA_func:
    def __init__(self, X, Y, pow=3, *, lamb=1e-7, to_truncate=True):
    ...
    def coeffs(self):
        ...
        Y = self.Y - Y0
        for xd in self.X.T:
            A = teneva.func_basis(xd, m=self.pow + 1, kind='cheb').T
            AtA = A.T @ A
            Aty = A.T @ Y

            cur_cf = sp.linalg.lstsq(AtA + self.lamb * np.identity(A.shape[1]), Aty,
                overwrite_a=True, overwrite_b=True, lapack_driver='gelsy')[0]
    ...
    def cores(self):
        ...
        A = teneva.delta(self.n, idx, cfs[0])
        for i, cf in enumerate(cfs[1:]):
            idx[:] = 0
            for pi, p in enumerate(cf):
                idx[i] = pi + 1
                A = teneva.add(A,
                   teneva.delta(self.n, idx, p)
                          )

        if self.to_truncate:
            return teneva.truncate(A, e=1e-8)
        else:
            return teneva.truncate(A)
